...
class AStar(object):
    ...
    def __init__(self, statespace_lo, statespace_hi, x_init, x_goal, occupancy, resolution=1, dist_norm=2):
    ...
    def is_free(self, x):
        """
        Checks if a give state x is free, meaning it is inside the bounds of the 
        map and is not inside any obstacle.
        Inputs:
            x: state tuple
        Output:
            Boolean True/False
        Hint: self.occupancy is a DetOccupancyGrid2D object, take a look at its 
        methods for what might be useful here
        """
        ########## Code starts here ##########
    ...
    def distance(self, x1, x2):
        ...
        """
        ########## Code starts here ##########
        return np.linalg.norm(np.array(x1) - np.array(x2), ord=self.dist_norm)
        ########## Code ends here ##########
    ...
    def get_neighbors(self, x):
        ...
        for dx in [-1, 0, 1]:
            for dy in [-1, 0, 1]:
                if dx == 0 and dy == 0:
                    continue
                neighbor = self.snap_to_grid((x[0] + dx * self.resolution, x[1] + dy * self.resolution))
    ...
    def find_best_est_cost_through(self):
        ...
        """
        # The min function is used to find the state in the open set with the lowest
        # estimated cost through it. The key parameter is a function that is used to
        # extract the value to be minimized from each element in the iterable. In this
        # case, the lambda function is used to extract the estimated cost through each
        # state. The lambda function takes a state as input and returns the estimated
        # cost through that state using the value stored in the est_cost_through
        # dictionary.
